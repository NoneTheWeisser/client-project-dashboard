...
export default function VolunteerKPI({
  title,
  monthlyReports = [],
  color = "black",
}) {
  ...
  const monthLabel = currentPeriod.month_start
    ? new Date(currentPeriod.month_start).toLocaleString("default", {
        month: "long",
      })
    : "";
  ...
  const ytdValue = monthlyReports
    ...
    .reduce((sum, r) => {
      if (title === "Total Volunteers")
        return sum + Number(r.total_volunteers ?? 0);
      return sum + Number(r.total_signups ?? 0);
    }, 0);

  return (
    <div className={`kpi-card outreach`}>
      <h4 className={`kpi-title outreach`} style={{ color }}>
        {title} — {monthLabel}
      </h4>
